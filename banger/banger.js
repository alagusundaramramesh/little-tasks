...
export default function Banger() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (window.innerWidth <= 768) {
        object1.style.transform = "translateY(-50%)";
        object2.style.transform = "translateY(-50%)";
        return;
      }
      ...
      if (rect.top < windowHeight && rect.bottom > 0) {
        ...
        const moveX = Math.min(progress * 600, 234);
  ...
  useEffect(() => {
    const alignSpotlight = () => {
      if (!stageRef.current || !characterTrackRef.current) return;

      const cards = characterTrackRef.current.querySelectorAll(".banger-character-card");
      if (!cards.length) return;

      const currentCard = cards[activeCharacter % cards.length];
      if (!currentCard) return;

      const stageRect = stageRef.current.getBoundingClientRect();
      const currentRect = currentCard.getBoundingClientRect();
      const currentCenterX = (currentRect.left + currentRect.right) / 2;
      const relativeX = currentCenterX - stageRect.left;

      setSpotlightLeft(relativeX);
    };

    alignSpotlight();
    window.addEventListener("resize", alignSpotlight);

    const timer = setTimeout(alignSpotlight, 0);

    return () => {
      clearTimeout(timer);
      window.removeEventListener("resize", alignSpotlight);
    };
  }, [activeCharacter]);
  ...
  const characterSettings = {
    ...
    cssEase: "ease-out",
    beforeChange: (_, next) => {
      setActiveCharacter(next % characterImages.length);
    }
  };
    ...
    <div className="banger-page">
      ...
      <section className="character-stage" ref={stageRef}>
        <div className="gallery-layout-player">
          ...
          <div className="image-div-player" style={{ backgroundImage: "url('/images/banger/1.jpg')" }}>
            <div className="container-fluid">
              <div className="row gallery-overlay">
                <img
                  ...
                  alt="Spotlight"
                  style={{ left: spotlightLeft !== null ? `${spotlightLeft}px` : undefined }}
                />
                <div className="col-lg-12 col-md-12 col-sm-12 banger-character-col">
                  <div className="character-container" ref={characterTrackRef}>
                    <Slider {...characterSettings}>
                      {characterImages.map((name, idx) => (
                        <div key={name}>
                          <div className={`banger-character-card ${idx === activeCharacter ? "is-active" : ""}`}>
                            <img src={`/images/banger/${name}.png`} alt={`Character ${idx + 1}`} />
                          </div>
                        </div>
                      ))}
                    </Slider>
                ...
                <div className="phone-slider">
                  <Slider {...phoneSettings}>
                    <div className="phone-slide">
                      <img src="/images/banger/Media 1.jpg" alt="Ice Land" />
                      <p>Ice Land</p>
                    </div>
                    <div className="phone-slide">
                      <img src="/images/banger/Media 2.jpg" alt="Taj Mahal" />
                      <p>Taj Mahal</p>
                    </div>
                    <div className="phone-slide">
                      <img src="/images/banger/Media 3.jpg" alt="Tower Bridge" />
                      <p>Tower Bridge</p>
                    </div>
                  </Slider>
                </div>
